# QA生成仕様書：Pythonのバージョン

## 1. 概要

txt形式のドキュメント(PythonのバージョンについてのWebページ)から事実を抽出し、4択のQA問題を自動生成してください。
...
- **【タイトル】**：Webページのタイトル
- **【見出し】**：Webページの見出し
- **【概要】**：Webページに書かれている内容の概要
- **【本文】**：コードバージョンの具体的な説明、section(New Features¶、New Modules¶など)に分かれて説明がされている
...
1. **コードバージョン名の組み込み**
   - 問題文に必ずコードバージョン名を組み込む
   - 形式例：「Python 3.12のxxx...」
   - バージョンは、【タイトル】【見出し】から抽出

2. **全てのsectionに対して問題作成**
   - 各sectionの内容から必ず1つ以上の問題を作成
   - sectionは「xxx¶」のように表記されている

2. **参照表現の禁止**
   - 「Webページによると」「本文から見ると」などの表現を使わない
   - 直接的な事実表現のみを使用
