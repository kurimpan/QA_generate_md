# QA生成仕様書：プログラミング言語 Yui

## 1. 概要

Markdown形式のドキュメント(プログラミング言語「Yui」の説明書）)から事実を抽出し、4択のQA問題を自動生成してください。
...
- **タイトル**：# で始まるドキュメント最初の見出し
- **本文**：# 以降のすべてのコンテンツ、## ごとにsectionが分かれている
...
1. **プログラミング言語の名称を組み込み**
   - 問題文に必ずプログラミング言語名を組み込む
   - 形式例：「Yui言語のxxx...」「Yui言語はxxx...」

2. **Yui言語についての問題を作成**
   - プログラミングにおける一般的な事実やPython言語についてではなく、Yui言語固有の問題を作成する
   - NG例：「クイックソートが使用するアルゴリズム設計手法はどれか？」

3. **参照表現の禁止**
   - 「このドキュメントによると」「本文から見ると」などの表現を使わない
   - 直接的な事実表現のみを使用
...
{
  "question_number": 1,
  "question_text": "2024年9月27日時点で、自民党の第28代総裁に選出されたのは誰か？",
  "options": {
    "A": "石破茂",
    "B": "高市早苗",
    "C": "茂木敏充",
